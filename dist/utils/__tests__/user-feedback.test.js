...
describe('ConsoleFeedback', () => {
    let feedback;
    beforeEach(() => {
        feedback = new user_feedback_1.ConsoleFeedback();
    });
    describe('success', () => {
        it('should log success message with green color and checkmark', () => {
            feedback.success('Operation completed');
            expect(mockConsole.log).toHaveBeenCalledWith(expect.stringContaining('✅ Operation completed'));
            expect(mockConsole.log).toHaveBeenCalledWith(expect.stringContaining('\x1b[32m') // Green color
            );
        });
    });
    describe('error', () => {
        it('should log error message with red color and X mark', () => {
            ...
            expect(mockConsole.error).toHaveBeenCalledWith(expect.stringContaining('❌ Something went wrong'));
            expect(mockConsole.error).toHaveBeenCalledWith(expect.stringContaining('\x1b[31m') // Red color
            );
        });
        it('should log stack trace in debug mode', () => {
            ...
            expect(mockConsole.error).toHaveBeenCalledWith(expect.stringContaining(error.stack));
    ...
    describe('warning', () => {
        it('should log warning message with yellow color and warning icon', () => {
            ...
            expect(mockConsole.warn).toHaveBeenCalledWith(expect.stringContaining('⚠️ This is a warning'));
            expect(mockConsole.warn).toHaveBeenCalledWith(expect.stringContaining('\x1b[33m') // Yellow color
            );
        });
    });
    describe('info', () => {
        it('should log info message with blue color and info icon', () => {
            ...
            expect(mockConsole.log).toHaveBeenCalledWith(expect.stringContaining('ℹ️ Information message'));
            expect(mockConsole.log).toHaveBeenCalledWith(expect.stringContaining('\x1b[34m') // Blue color
            );
        });
    });
    describe('progress', () => {
...
describe('FeedbackFactory', () => {
    it('should create console feedback by default', () => {
        const feedback = user_feedback_1.FeedbackFactory.create();
        expect(feedback).toBeInstanceOf(user_feedback_1.ConsoleFeedback);
    });
    ...
    it('should create silent feedback when specified', () => {
        const feedback = user_feedback_1.FeedbackFactory.create('silent');
        expect(feedback).toBeInstanceOf(user_feedback_1.SilentFeedback);
    });
});
describe('FeedbackUtils', () => {
    let silentFeedback;
    beforeEach(() => {
        silentFeedback = new user_feedback_1.SilentFeedback();
    });
    describe('withProgress', () => {
        it('should execute operation with progress and complete on success', async () => {
            ...
            const result = await user_feedback_1.FeedbackUtils.withProgress(operation, { total: 100, message: 'Testing...' }, silentFeedback);
        ...
        it('should fail progress and rethrow error on operation failure', async () => {
            ...
            await expect(user_feedback_1.FeedbackUtils.withProgress(operation, { total: 100, message: 'Testing...' }, silentFeedback)).rejects.toThrow('Operation failed');
            expect(operation).toHaveBeenCalledTimes(1);
        });
    });
    describe('withSteps', () => {
        it('should execute all steps with progress tracking', async () => {
            ...
            const results = await user_feedback_1.FeedbackUtils.withSteps(steps, silentFeedback);
        ...
        it('should fail and rethrow error if any step fails', async () => {
            ...
            await expect(user_feedback_1.FeedbackUtils.withSteps(steps, silentFeedback)).rejects.toThrow('Step 2 failed');
            expect(step1).toHaveBeenCalledTimes(1);
            expect(step2).toHaveBeenCalledTimes(1);
            expect(step3).not.toHaveBeenCalled(); // Should not reach step 3
        });
        it('should handle empty steps array', async () => {
            const results = await user_feedback_1.FeedbackUtils.withSteps([], silentFeedback);
...
describe('ConsoleProgressIndicator', () => {
    ...
    it('should complete progress with success message', () => {
        ...
        expect(mockConsole.log).toHaveBeenCalledWith(expect.stringContaining('✅ Completed in'));
    });
    it('should fail progress with error message', () => {
        ...
        expect(mockConsole.log).toHaveBeenCalledWith(expect.stringContaining('❌ Something went wrong'));
    });
});
//# sourceMappingURL=user-feedback.test.js.map