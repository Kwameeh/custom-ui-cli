...
class DocsCommand {
    constructor(dependencies) {
        this.name = 'docs';
        this.description = 'Show documentation for a component';
    ...
    async execute(args) {
        try {
            ...
            if (error instanceof types_1.CLIError) {
                ...
                const cliError = error_handler_1.ErrorHandler.createError(types_1.ERROR_CODES.REGISTRY_ERROR, `Failed to show documentation: ${error instanceof Error ? error.message : 'Unknown error'}`, { originalError: error });
     ...
     */
    async showComponentDocs(componentName, options) {
        ...
        const component = await error_handler_1.NetworkErrorHandler.withRetry(async () => {
            ...
            if (!comp) {
                throw error_handler_1.ErrorHandler.createError(types_1.ERROR_CODES.COMPONENT_NOT_FOUND, `Component "${componentName}" not found in registry`, { componentName, availableComponents: await this.getAvailableComponentNames() });
            }
            return comp;
        }, { componentName });
     ...
     */
    displayTextDocs(componentName, component, options) {
        ...
        if (component.metadata.dependencies.length > 0) {
            ...
            component.metadata.dependencies.forEach((dep) => {
                console.log(`  - ${dep}`);
            });
            console.log('');
        }
        ...
        if (component.metadata.npmDependencies.length > 0) {
            ...
            component.metadata.npmDependencies.forEach((dep) => {
                console.log(`  - ${dep}`);
            });
            console.log('');
        }
        // Files
        console.log('Files:');
        component.metadata.files.forEach((file) => {
     ...
     */
    displayJsonDocs(componentName, component) {
        const docs = {
            name: componentName,
     ...
     */
    displayExamples(componentName, component) {
     ...
     */
    displayComponentProps(componentName, component) {
        try {
     ...
     */
    generateBasicExample(componentName) {
