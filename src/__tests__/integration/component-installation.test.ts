...
describe('Component Installation Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    testProjectDir = path.join(tmpdir(), `custom-ui-test-${Date.now()}`);
...
export function cn(...inputs: ClassValue[]) {
  ...
  describe('Component Dependencies', () => {
    ...
    it('should validate component imports match dependencies', async () => {
      ...
      Object.entries(registry.components).forEach(([name, component]: [string, any]) => {
        ...
        if (content.includes('@radix-ui/react-')) {
          ...
          if (radixImports) {
            radixImports.forEach((radixImport: string) => {
