...
const MockedRegistryLoader = RegistryLoader as jest.MockedClass<typeof RegistryLoader>;

describe('RegistryUtils', () => {
  let registryUtils: RegistryUtils;
  let mockLoader: jest.Mocked<RegistryLoader>;

  beforeEach(() => {
    jest.clearAllMocks();
    mockLoader = new MockedRegistryLoader() as jest.Mocked<RegistryLoader>;
    registryUtils = new RegistryUtils();
    // Replace the internal loader with our mock
    (registryUtils as any).loader = mockLoader;
  });
  ...
  describe('getComponentDependencies', () => {
    beforeEach(() => {
      mockLoader.getComponent
        .mockImplementation((name: string) => Promise.resolve(mockComponents[name as keyof typeof mockComponents] || null));
    });
    ...
    it('should handle circular dependencies gracefully', async () => {
      ...
      mockLoader.getComponent
        .mockImplementation((name: string) => Promise.resolve(circularComponents[name as keyof typeof circularComponents] || null));
  ...
  describe('validateComponentDependencies', () => {
    ...
    it('should return invalid for component with missing dependencies', async () => {
      ...
      mockLoader.getComponent.mockImplementation((name: string) => {
        if (name === 'test') return Promise.resolve(componentWithMissingDep);
        return Promise.resolve(mockComponents[name as keyof typeof mockComponents] || null);
      });
